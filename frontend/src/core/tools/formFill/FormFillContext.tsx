...
export function FormFillProvider({
  ...
  const [modifiedFields, setModifiedFields] = useState<
    Map<string, Partial<ModifyFieldDefinition>>
  >(() => new Map());
  ...
  const updatePendingField = useCallback(
    (index: number, field: NewFieldDefinition) => {
      setCreationState((prev) => ({
        ...prev,
        pendingFields: prev.pendingFields.map((f, i) =>
          i === index ? field : f,
        ),
      }));
    },
  ...
  const setCreationDragRect = useCallback(
    (rect: FieldCreationState["dragRect"]) => {
  ...
  const updateFieldCoordinates = useCallback(
    (
      fieldName: string,
      coords: { x: number; y: number; width: number; height: number },
    ) => {
  ...
  const updateFieldProperties = useCallback(
    (fieldName: string, props: Partial<ModifyFieldDefinition>) => {
      setModifiedFields((prev) => {
        const next = new Map(prev);
        const existing = next.get(fieldName) || {};
  ...
  const hasUncommittedChanges =
    creationState.pendingFields.length > 0 || modifiedFields.size > 0;
