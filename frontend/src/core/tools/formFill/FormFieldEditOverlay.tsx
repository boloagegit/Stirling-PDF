...
export function FormFieldEditOverlay({
  ...
  const { scaleX, scaleY, pageHeightPts } = useMemo(() => {
    ...
    const firstWidget = pageFields
      .find((f) => f.widgets?.some((w) => w.pageIndex === pageIndex))
      ?.widgets?.find((w) => w.pageIndex === pageIndex);
    const cbHeight =
      firstWidget?.cropBoxHeight != null && firstWidget.cropBoxHeight > 0
        ? firstWidget.cropBoxHeight
        : undefined;
    return {
  ...
  const getFieldCssRect = useCallback(
    (field: FormField, widget: WidgetCoordinates) => {
      ...
      if (
        ...
        const cropBoxHeight =
          widget.cropBoxHeight != null && widget.cropBoxHeight > 0
            ? widget.cropBoxHeight
            : pageHeightPts;
  ...
  const handleFieldClick = useCallback(
    (e: React.PointerEvent, fieldName: string) => {
      e.stopPropagation();
      setEditState({
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (drag.type === "move") {
        newLeft = Math.max(
          0,
          Math.min(pageWidth - drag.startWidth, drag.startLeft + dx),
        );
        newTop = Math.max(
          0,
          Math.min(pageHeight - drag.startHeight, drag.startTop + dy),
        );
  ...
  return (
    ...
    >
      ...
      {pageFields.map((field) => {
        ...
        const usesPending =
          isSelected &&
          editState.pendingRect &&
          editState.interaction !== "idle";
        const rect = usesPending
          ? {
              left: editState.pendingRect!.x,
              top: editState.pendingRect!.y,
              width: editState.pendingRect!.width,
              height: editState.pendingRect!.height,
            }
        ...
        return (
          ...
          >
            ...
            {isSelected &&
              editState.interaction === "idle" &&
              HANDLE_POSITIONS.map(({ pos, cursor, top, left }) => (
                <div
                  key={pos}
                  style={{
                    position: "absolute",
